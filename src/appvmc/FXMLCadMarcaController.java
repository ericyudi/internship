 ...
 * @author eric-
 */
public class FXMLCadMarcaController implements Initializable {
  ...
  public void initialize(URL url, ResourceBundle rb) {
    ...
    private void fadeout() {
        ...
        Marca Pega = new Marca();
        List<Marca> res = Pega.get(filtro);
  ...
  @FXML
    ...
    private void clkBtApagar(ActionEvent event) {
        ...
        if (a.showAndWait().get() == ButtonType.OK) {
            Marca m;
            m = (Marca) tabela.getSelectionModel().getSelectedItem();
            m.apagar();
            carregaTabela("");
            txCod.setText("");
        }
    ...
    private void clkBtConfirmar(ActionEvent event) {
        ...
        if(!verifica()){
            ...
            Marca m = new Marca(cod, txNome.getText());
            if(m.getCod() == 0)
            {
                if(m.gravar()){
